...
import { useGetDepartments, Department, useDeleteDepartment } from '../departments.hooks'
...
const Departments = () => {
    ...
    const { data, isError, isLoading } = useGetDepartments();
    ...
    return (
        <>
                    ...
                    <Card>
                        <CardBody>
                            <div className="d-flex flex-wrap justify-content-between gap-3">
                                ...
                                <div>
                                    <Button onClick={() => navigate("/departments/create")} variant="success">
                      ...
                      <th className="border-0 py-2"> الحالة</th> */}
                                    ...
                                    <tbody>
                                        {!isError && !isLoading && data?.map((department: Department, idx) => (
                                            <tr key={idx}>
                                                {/* <td>
                          <Link to={`/departments/${department.id}`} className="fw-medium">
                            #{department.id}
                                                ...
                                                <td>
                                                    <img src={department.image} alt="user-img" className="avatar-md rounded-circle me-2" />
                                                </td>

                                                <td>
                                                    <div>
                                                        <h5 className="fs-14 mt-1 fw-normal">{department.name?.ar}</h5>
                                                    </div>
                                                </td>
            ...
            <Modal show={!!selectedDepartment} onHide={handleCloseModal} className="fade">
                ...
                <ModalBody>
                    <p>
                        تأكيد حذف قسم : <strong>{selectedDepartment?.name?.ar}</strong>
                    </p>
                </ModalBody>
                <ModalFooter>
                    ...
                    <Button variant="danger" onClick={() => {
                        // You can call delete mutation here
                        console.log('Delete', selectedDepartment);
                        handleCloseModal(selectedDepartment?.id);
                    }}>
                        تأكيد الحذف
                    </Button>
                </ModalFooter>
